...
OTP_CONTEXT_RE = re.compile(
    ...
    r"(?i)(?:otp|one\s*time\s*(?:password|pin)|verification\s*code|verify\s*code|login\s*code|security\s*code|activation\s*code|pin|password)\D{0,40}?\b(\d{4,8})\b|"
...
def main():
    ...
    set_code_conv = ConversationHandler(
        entry_points=[CommandHandler("set_code", set_code_start), CallbackQueryHandler(set_code_start, pattern='^set_code$')],
        states={SET_CODE: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_code_save)]},
        fallbacks=[]
    )
    ...
    auto_conv = ConversationHandler(
        entry_points=[CommandHandler("auto", auto_start), CallbackQueryHandler(auto_start, pattern='^auto$')],
        states={AUTO_FIREBASE: [MessageHandler(filters.TEXT | filters.Document.ALL, auto_firebase)]},
        fallbacks=[CommandHandler("cancel", lambda u, c: ConversationHandler.END)]
    )
